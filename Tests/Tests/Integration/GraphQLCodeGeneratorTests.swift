...
struct GraphQLCodeGeneratorTests {
    ...
    func generatedSubscriptionRequestsCannotEnableAutomaticPersistedOperations() async throws {
        ...
        for persistedOperations in [
            ...
            if case .registered(let manifestURL, let allowUnregisteredOperations) = persistedOperations {
                ...
                let operationProtocols = try String(
                    contentsOf: getVariantAPI.appending(path: "HTTPSupport/GraphQLOperation.swift"),
                    encoding: .utf8
                )
                ...
                for apiDirectory in [getVariantAPI, postVariantAPI] {
                    let encoders = try String(
                        contentsOf: apiDirectory.appending(path: "HTTPSupport/DefaultEncoders.swift"),
                        encoding: .utf8
                    )
                    ...
                    let encoderProtocols = try String(
                        contentsOf: apiDirectory.appending(path: "HTTPSupport/Encoders.swift"),
                        encoding: .utf8
                    )
                    #expect(
    ...
    private func generateSubscriptionAPI(
        ...
        persistedOperations: Configuration.Output.API.HTTPSupport.PersistedOperations? = .automatic,
        minifyDocument: Bool = true
    ) async throws {
        try await Codegen(
            .configuration(
