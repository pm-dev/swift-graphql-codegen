...
struct GraphQLCodeGeneratorTests {
    ...
    func createsPersistedOperationManifestWithoutGeneratingSwiftFiles(
        minifyDocument: Bool
    ) async throws {
        ...
        let schemaURL = directory.appending(path: "schema.graphqls", directoryHint: .notDirectory)
        try """
        type Query { version: String!, viewer: Viewer! }
        type Mutation { setVersion(version: String!): String! }
        ...
        subscription Ticks { ticks }
        """.write(
            to: operationsDirectory.appending(path: "Operations.graphql", directoryHint: .notDirectory),
            atomically: true,
            encoding: .utf8
        )

        let outputDirectory = directory.appending(path: "Generated", directoryHint: .isDirectory)
        var configuration = Configuration.configuration(
            ...
            output: .output(
                schema: .schema(
                    directory: outputDirectory.appending(path: "Schema", directoryHint: .isDirectory)
                ),
                documents: .documents(
                    directory: .directory(
                        outputDirectory.appending(path: "Documents", directoryHint: .isDirectory)
                    ),
                    operations: .operations(minifyDocument: minifyDocument)
                ),
                support: .support(
                    directory: outputDirectory.appending(path: "Support", directoryHint: .isDirectory),
                    HTTPSupport: .httpSupport(persistedOperations: .registered())
                )
            )
        )
        let codegen = Codegen(configuration)

        let manifest = try await codegen.persistedOperationManifest()
        #expect(!FileManager.default.fileExists(atPath: outputDirectory.path))
        #expect(manifest.format == "apollo-persisted-query-manifest")
        #expect(manifest.version == 1)

        let mutationBody = minifyDocument
            ? "mutation SetVersion{setVersion(version:\"2\")}"
            : "mutation SetVersion { setVersion(version: \"2\") }"
        let subscriptionBody = minifyDocument
            ? "subscription Ticks{ticks}"
            : "subscription Ticks { ticks }"
        #expect(manifest.operations.contains { $0.body == mutationBody })
        #expect(manifest.operations.contains { $0.body == subscriptionBody })
        ...
        configuration.output.support.HTTPSupport = nil
        let manifestWithoutHTTP = try await Codegen(configuration).persistedOperationManifest()
        #expect(manifestWithoutHTTP.operations.map(\.id) == manifest.operations.map(\.id))
        #expect(!FileManager.default.fileExists(atPath: outputDirectory.path))

        let manifestURL = directory.appending(
            path: "Manifests/operations.json",
            directoryHint: .notDirectory
        )
        ...
        #expect(savedManifest.operations.map(\.id) == manifest.operations.map(\.id))
        #expect(!FileManager.default.fileExists(atPath: outputDirectory.path))

        try await codegen.run()
        let generatedPaths = try FileManager.default.subpathsOfDirectory(atPath: outputDirectory.path)
        #expect(!generatedPaths.contains { $0.hasSuffix(".json") })
        ...
        """.write(
            to: operationsDirectory.appending(path: "ViewerProfiles.graphql", directoryHint: .notDirectory),
            atomically: true,
            encoding: .utf8
        )
        let manifestWithSwiftTypeConflict = try await codegen.persistedOperationManifest()
        #expect(manifestWithSwiftTypeConflict.operations.contains { $0.name == "ViewerProfiles" })
    }
